...
class TurtleStrategy(bt.Strategy):
    ...
    def __init__(self):
        print('long_period:',self.p.long_period)
        print('short_period:',self.p.short_period)
        print('ATR_period:',self.p.ATR_period)
        print('CASH_rate:',self.p.CASH_rate)
        print('ATR_jiacang:',self.p.ATR_jiacang)
        self.order = None
        self.buyprice = 0
        self.buycomm = 0
        self.buy_size = 0
...
def runstrategy():
    ...
    ('long_period', 20),
              ...
              ('ATR_jiacang',1),
    '''
    cerebro.optstrategy(TurtleStrategy, long_period=range(10, 26), short_period=range(3, 11), ATR_period=range(2, 6),
                        CASH_rate=np.arange(0.03, 0.5, 0.1), ATR_jiacang=np.arange(0.5, 2.1, 0.5))
    ...
    for run in opt_runs:
        for strategy in run:
            ...
            PnL = round((value - args.cash)/args.cash, 2)
            long_period = strategy.params.long_period
            short_period=strategy.params.short_period
            ATR_period=strategy.params.ATR_period
            CASH_rate=strategy.params.CASH_rate
            ATR_jiacang=strategy.params.ATR_jiacang



            final_results_list.append([long_period,short_period,ATR_period,CASH_rate,ATR_jiacang, PnL])
    ...
    for result in by_PnL:
        print('Long_Period: {},Short_Period:{},ATR_period:{},CASH_rate:{},ATR_jiacang:{}, PnL: {}'.format(result[0],result[1],result[2],result[3],result[4],result[5]))
