...
class TurtleStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        for i, d in enumerate(self.datas):
            ...
            # 入场：价格突破上轨线且空仓时
            self.buyflag[d]=True
            if d.lines.close[0] == d.lines.open[0] == d.lines.high[0] == d.lines.low[0]:
                self.buyflag[d] = False

            for i in range(1, self.p.N_day + 1):
                if d.lines.close[-i] * (1 + self.p.danrihuiche) < d.lines.high[-i]:
                    self.buyflag[d] = False
            p1 = max(d.lines.close.get(ago=-1, size=self.p.N_day))

            if (p1 < d.lines.close[0] < p1 * (1 + self.p.up_rate)) & (
                    np.mean(d.lines.volume.get(ago=-1, size=self.p.N_day)) / np.mean(
                    d.lines.volume.get(ago=-self.p.N_day, size=self.p.N_day)) > self.p.volup) & (
                    self.buy_count[d] == 0) & (self.buyflag[d] == True):
                ...
                self.buy_count[d] = 1
                print(d.datetime.date(0), 'buy ', d.stock_code[0], 'size:', self.buy_size)
                self.order = self.buy(data=d,size=self.buy_size)
...
def runstrategy():
    ...
    stock_list = ['600036','601633', '300015', '601318', '600161','603755','300785','600030',\
                  '600161','002007','601088','601933','603596','603348','002074','002970','300496','600009']
