...
class MomentumInd(bt.Indicator):
    ...
    def next(self):
        preprice = self.data.get(-self.params.period)[0]
        self.lines.year_rate[0] = (self.data.get(0)[0] - preprice) / preprice


class FIP_up(bt.Indicator):
    lines = ('up_days',)
    params = (('period', 250),)
    ...
    def next(self):
        allprice = pd.DataFrame(self.data.get(size=self.params.period))
        allprice = allprice.pct_change()
        allprice[allprice > 1e-4] = 1
        allprice[allprice < 1e-4] = 0
        self.lines.up_days[0] = allprice.sum()


class FIP_down(bt.Indicator):
    lines = ('down_days',)
    params = (('period', 250),)

    def __init__(self):
        self.addminperiod(self.params.period + 1)
        #self.allprice=pd.DataFrame(self.data)
        #print(self.allprice)
    ...
    def next(self):
        ...
        allprice[allprice > -1e-4] = 1
        allprice[allprice < -1e-4] = 0
        self.lines.down_days[0] = allprice.sum()
...
class Momentum(bt.Strategy):
    ...
    def __init__(self):
        ...
        for d in self.stocks:
            self.inds[d] = {}
            self.inds[d]["momentumInd"] = MomentumInd(d.close, period=self.params.period)
            self.inds[d]["up"] = FIP_up(d.close, period=self.params.period)
            self.inds[d]["down"] = FIP_down(d.close, period=self.params.period)
            self.inds[d]["FIP"] = self.inds[d]["momentumInd"] * (self.inds[d]["down"] - self.inds[d]["up"])
            self.chicang[d] = {}
            for i in range(self.params.chicang_month):
                self.chicang[d][str(i + 1)] = 0
        print('init end')
    ...
    def next(self):
        ...
        if self.starts % self.params.month_days == 0:
    ...
    def rebalance_portfolio(self):
        ...
        self.rankings = list(filter(lambda d: len(d) > self.params.period, self.stocks))
        self.rankings.sort(key=lambda d: self.inds[d]["momentumInd"][0],reverse=True)
        self.FIPs = self.rankings[:self.params.firstSelect].copy()
        self.FIPs.sort(key=lambda d: self.inds[d]["FIP"][0])
        self.FIPs=self.FIPs[:self.params.secondSelect]


        # sell stocks based on criteria
        for i, d in enumerate(self.rankings):
            if self.chicang[d][str(self.counts)] > 0:
                if d not in set(self.FIPs):
                    print('sell:',d.datetime.date(0),d.stock_code[0],d.open[1],self.chicang[d][str(self.counts)])
                    self.close(d, size=self.chicang[d][str(self.counts)])
                    self.chicang[d][str(self.counts)]=0


        # buy stocks with remaining cash
        for i, d in enumerate(self.FIPs[:self.params.secondSelect]):
            ...
            if self.chicang[d][str(self.counts)] == 0:
                size = value * 0.03
                print('buy:',d.datetime.date(0),d.stock_code[0],d.open[1],size)
                self.chicang[d][str(self.counts)] = size
                self.buy(d, size=size)

        self.counts += 1
        if self.counts > self.params.chicang_month:
            self.counts = 1
...
def runstrategy():
    ...
    stock_list = ['600036','601633', '300015', '601318', '600161','603755','300785','600030','600161','002007','601088','601933','603596','603348','002074','002970','300496']

    #stock_list = ['600036', '601633']
    ...
    for i, stock_code in enumerate(stock_list):
        ...
        stock_daily_info['trade_date'] = stock_daily_info['trade_date'].apply(
        ...
        stock_daily_info.rename(
            columns={'trade_date': 'datetime', 'qfq_open': 'open', 'qfq_high': 'high', 'qfq_low': 'low',
