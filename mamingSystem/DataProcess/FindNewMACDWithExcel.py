...
def calculateMACD(closeArray, shortPeriod=12, longPeriod=26, signalPeriod=9):
    macd, diff, dea=ta.MACD(closeArray, fastperiod=shortPeriod, slowperiod=longPeriod, signalperiod=signalPeriod)
...
def CalcStockMACD(Stockcode,startday,endday,stockInfo):
    args = parse_args()
    ...
    stockInfo['datetime']=stockInfo['datetime'].apply(lambda x: x[0:4] + '-' + x[4:6] + '-' + x[6:])
    orders=['datetime','stock_code','open','high','low','close','volume','outstanding_share','turnover']
    stockInfo=stockInfo[orders]
    ...
    #print(stockInfo)
    #stock_daily_info=stock_daily_info+stockInfo
    if str(stockInfo.iloc[0,5])==' —':
        return False
    stock_daily_info=stock_daily_info.append(stockInfo,ignore_index=True)
    ...
    #print(stock_daily_info)
    (macd, diff, dea)=calculateMACD(stock_daily_info['close'])

    lenSize=len(macd)

    if macd[lenSize-1]>0 and diff[lenSize-1]>0 and dea[lenSize-1]>0:
        if macd[lenSize-2]<=0 or diff[lenSize-2]<=0 or dea[lenSize-2]<=0:
            return True
        else:
            return False
    else:
...
if __name__ == '__main__':
    ...
    mark = False
    stocklist=[]
    StockExcel=pd.read_excel('../data/20220104.xlsx',converters={u'代码':str})
    #print(StockExcel)
    for code, name in zip(stock_info_a_code_name_df['code'], stock_info_a_code_name_df['name']):
        print(code, name)
        stockInfos=StockExcel[StockExcel['代码']==code]

        stockInfo=stockInfos[['代码','开盘','最高','最低','最新','市盈率','涨跌','换手%']]
        stockInfo['trade_date']=appendDay
        stockInfo.rename(
            columns={'代码':'stock_code','trade_date':'datetime','开盘': 'open', '最高': 'high', '最低': 'low',
                     '最新': 'close','市盈率':'volume','涨跌':'outstanding_share','换手%':'turnover'}, inplace=True)
        stockInfo.reset_index(drop=True)
        #print(stockInfo)
        ...
        if CalcStockMACD(code,startday,endday,stockInfo):
            print('find one:',code,name)
