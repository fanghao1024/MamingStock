...
def FindHammer(Stockcode,startday,endday):
    args = parse_args()
    ...
    if stock_daily_info['close'][0] > stock_daily_info['open'][0]:
        shang = stock_daily_info['close'][0]
        xia = stock_daily_info['open'][0]
    else:
        xia = stock_daily_info['close'][0]
        shang = stock_daily_info['open'][0]

    if (xia-stock_daily_info['low'][0])>2.5*(shang-xia) and (stock_daily_info['high'][0]-shang)<(shang-xia):
        return True
    else:
        return False
...
if __name__ == '__main__':
    ...
    for code, name in zip(stock_info_a_code_name_df['code'], stock_info_a_code_name_df['name']):
        ...
        if FindHammer(code,startday,endday):
