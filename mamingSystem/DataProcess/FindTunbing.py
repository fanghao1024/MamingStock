...
def FindTunbing(Stockcode,startday,endday):
    args = parse_args()
    ...
    if len(stock_daily_info['close'])<2:
        return 0

    if stock_daily_info['close'][-2] >= stock_daily_info['open'][-2] and stock_daily_info['close'][-1] < stock_daily_info['open'][-1] \
            and stock_daily_info['open'][-1]>stock_daily_info['close'][-2] and stock_daily_info['open'][-2]>stock_daily_info['close'][-1]:
        return 1

    if stock_daily_info['close'][-2] <= stock_daily_info['open'][-2] and stock_daily_info['close'][-1] > \
            stock_daily_info['open'][-1] \
            and stock_daily_info['open'][-1] < stock_daily_info['close'][-2] and stock_daily_info['open'][-2] < \
            stock_daily_info['close'][-1]:
        return 2
...
if __name__ == '__main__':
    ...
    for code, name in zip(stock_info_a_code_name_df['code'], stock_info_a_code_name_df['name']):
        ...
        if str(code) == '689009' or str(code) == '688616':
            continue
        marks=FindTunbing(code,startday,endday)
        if marks==1:
            print('find one down:',code,name)
            stocklist_down.append(code)
        if marks==2:
            print('find one up:',code,name)
            stocklist_up.append(code)
    print('-----stock list down-----')
    print(stocklist_down)
