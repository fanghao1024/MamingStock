...
def FindEveningStar(Stockcode,startday,endday):
    args = parse_args()
    ...
    if stock_daily_info['close'][-3] > stock_daily_info['open'][-3] and stock_daily_info['low'][-2] > stock_daily_info['high'][-3] and \
		stock_daily_info['close'][-1]<stock_daily_info['open'][-1] and (stock_daily_info['close'][-3]-stock_daily_info['close'][-1])/(stock_daily_info['close'][-3]-stock_daily_info['open'][-3])>0.5 :
        return 1
...
if __name__ == '__main__':
    ...
    for code, name in zip(stock_info_a_code_name_df['code'], stock_info_a_code_name_df['name']):
        ...
        if str(code) == '689009' or str(code) == '688616':
            continue
        marks=FindEveningStar(code,startday,endday)
        if marks==1:
