...
def transferToWeeklyLine(data, types):
	data['date'] = data['date'].apply(lambda x: x[0:4] + '-' + x[4:6] + '-' + x[6:])
	...
	period_stock_data.rename(columns={'datetime': 'date'}, inplace=True)
	print(period_stock_data)
	return period_stock_data


# 获得页面数据。
class GetDataWeeklyLineHandler(webBase.BaseHandler):
	@gen.coroutine
	def get(self):
		...
		try:
			...
			date_start = (date_now + datetime.timedelta(days=-1000)).strftime("%Y-%m-%d")
			...
			print(stock)
			stock=transferToWeeklyLine(stock,transfer_type)
			print('after:')
			print(stock)
...
def plot_EMA(stockStat, conf):
	...
	for key, val in enumerate(conf["dic"]):
		...
		if key != 0:
			# p1 = figure(width=1000, height=250, x_axis_type="datetime",title=val)
			p1.line(stockStats.index, stockStat[val], color=line_Color[key], legend_label='EMA' + val.split('_')[1],
					width=3)
	...
	p1.line(stockStats.index, vol_10, color='Fuchsia', legend_label='vol_sma_10', width=3)
	p1.legend.location = "top_left"
	p1.x_range = p_list[0][0].x_range
	p_list.append([p1])
...
def plot_ATR(stockStat, conf):
	...
	p1.line(stockStats.index, stockStat["close_22_ema"] + stockStat['atr_22'], legend_label="up_atr_22",
			color=line_Color[2], width=3)
	p1.line(stockStats.index, stockStat["close_22_ema"] - stockStat['atr_22'], legend_label="down_atr_22",
			color=line_Color[3], width=3)
...
def plot_KDJ(stockStat, conf):
	...
	p1.line(stockStats.index, stockStat['kdjj'], color=line_Color[2], legend_label='J', width=2)
