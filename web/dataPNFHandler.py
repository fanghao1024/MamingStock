...
# 获得页面数据。
class GetDataPNFHandler(webBase.BaseHandler):
	@gen.coroutine
	def get(self):
		...
		try:
			...
			buf=io.BytesIO()
			mpf.plot(stock, type='pnf', style=CN, pnf_params=dict(box_size='atr', atr_length=10, reversal=2),
					 mav=(5, 10, 20), volume=True, panel_ratios=(3, 1), figscale=1.9, axtitle=types + code,xrotation=10,
					 scale_padding={'left': 0.3, 'top': 0.3, 'right': 0.2, 'bottom': 0.3},savefig=buf)
			photo_data=base64.b64encode(buf.getbuffer()).decode('ascii')
			#print('url:',f"<img src='data:image/png;base64,{photo_data}' align='middle'/>")
			comp_list.append({"img": f"<img src='data:image/png;base64,{photo_data}'/>","title": 'pnf',	"desc": 'pnf'})
		...
		self.render("stock_pnf.html", comp_list=comp_list,
