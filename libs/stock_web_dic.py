...
STOCK_WEB_DATA_LIST.append(
	...
	#		columns=['date','code','name','latest_price','quote_change','ups_downs','volume','turnover','amplitude','high','low','open','closed','quantity_ratio','turnover_rate','pe_dynamic','pb'],
	...
	StockWebData(
			...
			type="1，基本数据及指标",
			name="股票数据",
			table_name="dailyline",
			columns=['date', 'code','name', 'qfq_open', 'qfq_high', 'qfq_low', 'qfq_close', 'volume', 'deal', 'zhenfu',
					 'zhangdiefu', 'zhangdiee', 'turnover'],
			column_names=['日期', '代码','名称', '开盘', '最高', '最低', '收盘', '成交量', '成交额', '振幅', '涨跌幅', '涨跌额', '换手率'],
			primary_key=[],
			order_by=" code asc "
		)

)
# 指数数据  代码  名称  最新价 涨跌额 涨跌幅 昨收 今开 最高  最低  成交量   成交额
STOCK_WEB_DATA_LIST.append(
	StockWebData(
		mode="query",
		type="1，基本数据及指标",
		name="指数数据",
		table_name="indexline",
		columns=['date', 'code', 'name', 'open', 'close', 'high', 'low', 'volume', 'amount'],

		column_names=['日期', '代码', '名称', '开盘', '最高', '最低','收盘', '成交量', '成交额'],
		primary_key=[],
		order_by=" code asc "
	)
)
...
STOCK_WEB_DATA_LIST.append(
	StockWebData(
		...
		type="1，基本数据及指标",
		name="行业板块数据",
		table_name="industryline",
		columns=['date', 'code', 'name', 'qfq_open', 'qfq_close', 'qfq_high', 'qfq_low', 'volume', 'amount','zhangdiefu','zhangdiee','zhenfu','turnover'],

		column_names=['日期', '代码', '名称', '开盘', '最高', '最低','收盘', '成交量', '成交额','涨跌幅','涨跌额','振幅','换手率'],
		primary_key=[],
		order_by=" code asc "
	)
)
...
STOCK_WEB_DATA_LIST.append(
	StockWebData(
		...
		columns=['date', 'code', 'name', 'open', 'close', 'high', 'low', 'volume'],

		column_names=['日期', '代码', '名称', '开盘', '最高', '最低','收盘', '成交量'],
		primary_key=[],
		order_by=" code asc "
	)
)
...
for tmp in STOCK_WEB_DATA_LIST:
	try:
		...
		tmp.column_names.insert(tmp_idx, "查看股票")
